...
// --- Pinos ---
// const int pinoTouch = 14;  // DESATIVADO
const int pinoBuzzer = 12;    // D6
const int pinoBotaoFeed = 13; // D7 ALIMENTAR / SALTAR
const int pinoBotaoGame = 0;  // D3 JOGAR
...
// Variáveis de Input (Debounce dos Botões)
...
// =========================================================================
// FUNÇÃO PARA SALVAR E CARREGAR (MEMÓRIA)
// =========================================================================
void saveData() {
  Serial.println("A salvar estado na EEPROM...");
  TamagotchiState state = {
    energia, felicidade, fome, high_score,
    0xAD 
  };
  EEPROM.put(0, state);
  EEPROM.commit();
  Serial.println("Estado salvo!");
}

void loadData() {
  Serial.println("A tentar carregar estado da EEPROM...");
  TamagotchiState state;
  EEPROM.begin(sizeof(TamagotchiState));
  EEPROM.get(0, state);
  ...
  if (state.magicByte == 0xAD) {
    ...
    Serial.println("Estado anterior carregado!");
  } else {
    Serial.println("Memoria incompativel ou vazia. A reiniciar...");
...
void draw_happy_eyes_replacement() {
  tone(pinoBuzzer, 988, 100); delay(100);
  tone(pinoBuzzer, 1318, 150); 
  display.clearDisplay();
  drawInterface();
  int centerY = (SCREEN_HEIGHT - UI_HEIGHT) / 2 + UI_HEIGHT;
...
void wakeup() {
  estaDormindo = false;
  tone(pinoBuzzer, 1046, 100); delay(120); 
  tone(pinoBuzzer, 1046, 100); delay(120);
  tone(pinoBuzzer, 1046, 100); delay(120);
  tone(pinoBuzzer, 1318, 400); delay(400);
  noTone(pinoBuzzer);
  center_eyes(false);
...
void happy_eye() {
  tone(pinoBuzzer, 1046, 50); delay(50); tone(pinoBuzzer, 1318, 50); delay(50); tone(pinoBuzzer, 1568, 100);
  center_eyes(false);
  draw_eyes(false);
...
<head>
  ...
  <style>
    ...
    .stats { background-color: #34495e; padding: 10px; 
    border-radius: 8px; margin-bottom: 20px; width: 90%; max-width: 400px; text-align: center; font-size: 1.1em; }
    .button-grid { display: grid;
    grid-template-columns: 1fr 1fr; gap: 15px; max-width: 400px; width: 90%; }
    button { background-color: #3498db; color: white;
    border: none; border-radius: 8px; padding: 20px; font-size: 1.2em; cursor: pointer; transition: background-color 0.3s;
    }
    button:hover { background-color: #2980b9; }
    .full-width { grid-column: 1 / -1;
    }
    .btn-mic { background-color: #e74c3c; font-weight: bold; }
    .btn-mic.listening { background-color: #c0392b;
    ...
    #voice-status { font-size: 0.9em; color: #f1c40f; margin-top: 5px;
    min-height: 20px;}
  </style>
</head>
<body>
  ...
  <div style="width: 90%; max-width: 400px; margin-bottom: 10px; text-align:center;">
    ...
    <button class="full-width btn-test" onclick="testAudio()">Testar Som</button>
  </div>

  <div class="stats">
    E: <span id="energia">--</span>% |
    H: <span id="felicidade">--</span>% | F: <span id="fome">--</span>%
  </div>
  
  <div class="button-grid">
    <button data-action="jogar" class="full-width" style="background-color:#e67e22">Jogar</button>
    <button data-action="alimentar" class="full-width" style="background-color:#27ae60">Comer</button>
  ...
  <script>
    ...
    function getStats(){
      fetch('/stats').then(r => r.json()).then(d => {
        ...
        switch (d.emocao) {
          case "faminto": case "triste": face = "(T_T)";
          break;
...
String json = "{\"energia\":" + String(energia) + ", \"felicidade\":" + String(felicidade) + 
                ", \"fome\":" + String(fome) + ", \"emocao\":\"" + currentEmotionString + 
                "\", \"hora\":\"" + timeString + "\"}";
...
void handleWakeup() { estaDormindo = false; requestedAnimation = 1;
server.send(200, "text/plain", "OK"); }
...
void setup() {
  Serial.begin(115200);
  // pinMode(pinoTouch, INPUT); // DESATIVADO
  pinMode(pinoBuzzer, OUTPUT);
  pinMode(pinoBotaoFeed, INPUT_PULLUP); // <-- BOTÃO 1
  pinMode(pinoBotaoGame, INPUT_PULLUP); // <-- BOTÃO 2
  Wire.begin(4, 5);
  ...
  timeClient.begin();
  // --- CORREÇÃO DE ORDEM DE NASCIMENTO ---
  ...
  lastInteractionTime = millis();
  center_eyes(true);
  tone(pinoBuzzer, 523, 100); delay(100); tone(pinoBuzzer, 784, 150); noTone(pinoBuzzer);
}
...
void game_over() {
  ...
  lastInteractionTime = millis();
  // Voltar do jogo conta como interação
  noTone(pinoBuzzer);
}

void game_loop() {
  // Lógica de Salto
  int jumpState = digitalRead(pinoBotaoFeed);
  if (jumpState == LOW && !player_is_jumping) {
...
void tamagotchi_loop() {
  ...
  // Botão 1: Alimentar
  if (digitalRead(pinoBotaoFeed) == LOW && (currentMillis - lastInputTime > inputCooldown)) {
    lastInputTime = currentMillis;
    handleAlimentar();
  }
  
  // Botão 2: Jogar
  if (digitalRead(pinoBotaoGame) == LOW && (currentMillis - lastInputTime > inputCooldown)) {
    lastInputTime = currentMillis;
    handleJogar();
  }
  ...
  // ===========================================================
    ...
    if (currentHour >= 22 || currentHour < 8) {
      ...
      delay(1000);
      return;
      // Sai da função para não executar o resto
    }
